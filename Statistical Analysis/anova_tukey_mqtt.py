...
for flow in root.findall(".//FlowStats/Flow"):
    ...
    delay_sum_ns = float(flow.attrib["delaySum"].replace("+", "").replace("ns", ""))
    jitter_sum_ns = float(flow.attrib["jitterSum"].replace("+", "").replace("ns", ""))
    time_first_tx_ns = float(flow.attrib["timeFirstTxPacket"].replace("+", "").replace("ns", ""))
    time_last_rx_ns = float(flow.attrib["timeLastRxPacket"].replace("+", "").replace("ns", ""))
