...
int main(int argc, char *argv[]) {
   ...
   // Initialize blockchain on each node
    ...
    phy.EnablePcap("wifi_ap_blocksec_ddos", wifiApNode); // Capture WAP traffic
    phy.EnablePcap("baxter_pump_blocksec_ddos", wifiDevices.Get(0)); // Capture Baxter traffic (Node 0)
    phy.EnablePcap("hexoskin_phone_blocksec_ddos", wifiDevices.Get(1)); // Capture Hexoskin traffic (Node 1)
    p2p.EnablePcap("hexoskin_shirt_blocksec_ddos", p2pDevices); // Hexoskin Shirt device (second device in p2pDevices)
    ...
    monitor->SerializeToXmlFile("flowmonitor-stats_blocksec_ddos.xml", true, true);
