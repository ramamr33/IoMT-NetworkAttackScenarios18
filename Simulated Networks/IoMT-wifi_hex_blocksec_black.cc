...
NS_LOG_COMPONENT_DEFINE("IoMTDetailedNetworkWithAPAndMITM");
class Block {
public:
...
bool MitmInterceptAndDrop(Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from) {
    ...
    if (protocol == 17 && from == InetSocketAddress("10.1.1.2", 9090)) { // UDP packet to Hexoskin Shirt
...
void SetupMitmAttack(NodeContainer &attackerNode, NodeContainer &wifiNodes, Ipv4InterfaceContainer &wifiInterfaces) {
    ...
    if (attackerRouting) {
        Ipv4Address targetAddress = wifiInterfaces.GetAddress(1); // Target Hexoskin device
...
int main(int argc, char *argv[])
{
    ...
    phy.EnablePcap("baxter_pump_black", wifiDevices.Get(0));  // Capture Baxter traffic (Node 0)
    phy.EnablePcap("hexoskin_phone_black", wifiDevices.Get(1));  // Capture Baxter traffic (Node 0)
    ...
    monitor->SerializeToXmlFile("flowmonitor-stats_black.xml", true, true);
